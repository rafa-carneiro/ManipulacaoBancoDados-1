...
1. Quais são as estatísticas suficientes para a determinação do percentual de vôos atrasados na chegada (\colorbox{codegray}{$\symtt{ARRIVAL\_DELAY}$} > 10)?
...
\begin{center}              
   O Número Total de Voos no conjunto de dados $(n)$ e a Contagem de Voos cujo valor na coluna \colorbox{codegray}{$\symtt{ARRIVAL\_DELAY}$} seja maior que 10 $(x)$. Com essas duas estatísticas, o percentual $p$ é calculado por $p = (x / n) * 100$
\end{center}
...
2. Crie uma função chamada \colorbox{codegray}{$\symtt{getStats}$} que, para um conjunto de qualquer tamanho de dados provenientes de \colorbox{codegray}{$\symtt{flights.csv.zip}$}, execute as seguintes tarefas (usando apenas verbos do \colorbox{codegray}{$\symtt{dplyr}$}):
...
d) Para cada grupo em b), determine as estatísticas suficientes apontadas no item 1. e os retorne como um objeto da classe \colorbox{codegray}{$\symtt{tibble}$};

e) A função deve receber apenas dois argumentos: 

   i. \colorbox{codegray}{$\symtt{input}$}: o conjunto de dados (referente ao lote em questão);
  
   ii. \colorbox{codegray}{$\symtt{pos}$}: argumento de posicionamento de ponteiro dentro da base de dados. Apesar de existir na função, este argumento não será empregado internamente. É importante observar que, sem a definição deste argumento, será impossível fazer a leitura por partes.
...
3. Utilize alguma função \colorbox{codegray}{$\symtt{readr::read\_***\_chunked}$} para importar o arquivo \colorbox{codegray}{$\symtt{flights.csv.zip}$}.

a) Configure o tamanho do lote (chunk) para 100 mil registros;

b) Configure a função de \colorbox{codegray}{$\symtt{callback}$} para instanciar DataFrames aplicando a função \colorbox{codegray}{$\symtt{getStats}$} criada acima;

c) Configure o argumento \colorbox{codegray}{$\symtt{col\_types}$} de forma que ele leia, diretamente do arquivo, apenas as colunas de interesse (veja nota de aula para identificar como realizar esta tarefa);
...
\begin{center}
   $\Delta t=\symtt{t1-t0}=$ `r paste0(difftime(t1,t0,units = "secs"))` $s$
\end{center}
...
4. Crie uma função chamada \colorbox{codegray}{$\symtt{computeStats}$} que:
...
b) Retorne as informações em um \colorbox{codegray}{$\symtt{tibble}$} contendo apenas as seguintes colunas: 

   i. \colorbox{codegray}{$\symtt{Cia}$}: sigla da companhia aérea;
   
   ii. \colorbox{codegray}{$\symtt{Data}$}: data, no formato AAAA-MM-DD (dica: utilize o comando \colorbox{codegray}{$\symtt{as.Date}$});
   
   iii. \colorbox{codegray}{$\symtt{Perc}$}: percentual de atraso para aquela cia. aérea e data, apresentado como um número real no intervalo $[0,1]$.
...
a) Instale e carregue os pacotes \colorbox{codegray}{$\symtt{ggcal}$} e \colorbox{codegray}{$\symtt{ggplot2}$}.

b) Defina uma paleta de cores em modo gradiente. Utilize o comando \colorbox{codegray}{$\symtt{scale\_fill\_gradient.}$} A cor inicial da paleta deve ser \colorbox{4575b4}{\textcolor{white}{$\symtt{\#4575b4}$}} e a cor final, \colorbox{d73027}{\textcolor{white}{$\symtt{\#d73027}$}}. A paleta deve ser armazenada no objeto \colorbox{codegray}{$\symtt{pal}$}.

c) Crie uma função chamada \colorbox{codegray}{$\symtt{baseCalendario}$} que recebe 2 argumentos a seguir: \colorbox{codegray}{$\symtt{stats}$} (\colorbox{codegray}{$\symtt{tibble}$} com resultados calculados na questão 4) e \colorbox{codegray}{$\symtt{cia}$} (sigla da Cia. Aérea de interesse). A função deverá: 

   i. Criar um subconjunto de \colorbox{codegray}{$\symtt{stats}$} de forma a conter informações de atraso e data apenas da Cia. Aérea dada por cia.
   
   ii. Para o subconjunto acima, montar a base do calendário, utilizando \colorbox{codegray}{$\symtt{ggcal(x,y)}$}. Nesta notação, \colorbox{codegray}{$\symtt{x}$} representa as datas de interesse e \colorbox{codegray}{$\symtt{y}$}, os percentuais de atraso para as datas descritas em \colorbox{codegray}{$\symtt{x}$}.
   
   iii. Retornar para o usuário a base do calendário criada acima.
   
d) Executar a função \colorbox{codegray}{$\symtt{baseCalendario}$} para cada uma das Cias. Aéreas e armazenar os resultados, respectivamente, nas variáveis: \colorbox{codegray}{$\symtt{cAA}$}, \colorbox{codegray}{$\symtt{cDL}$}, \colorbox{codegray}{$\symtt{cUA}$} e \colorbox{codegray}{$\symtt{cUS}$}.

e) Para cada uma das Cias. Aéreas, apresente o mapa de calor respectivo utilizando a combinação de camadas do \colorbox{codegray}{$\symtt{ggplot2}$}. Lembre-se de adicionar um título utilizando o comando \colorbox{codegray}{$\symtt{ggtitle}$}. Por exemplo, \colorbox{codegray}{$\symtt{verb01}$}.
...
\begin{center}
   $\Delta t=$ `r paste0(difftime(t1,t0,units = "secs"))` $s$
\end{center}
...
\hfill\pagebreak
## § Resultados e Comparações (R--Python)
...
\begin{center}
   Tabela 4: Média de vôos atrasados por Cia estudada e dia (Obtida através do Python)
\end{center}
...
\begin{center}
   Tabela 3: Média de vôos atrasados por Cia estudada e dia.
\end{center}
...
\begin{center}
   Tabela 5: Média de vôos atrasados por Cia estudada (Obtida através do Python)
\end{center}
...
\hfill\pagebreak
## § Resultados e Comparações (R--Julia)
...
\begin{center}
   Tabela 6: Média de vôos atrasados por Cia estudada e dia (Obtida através do Julia)
\end{center}

\centering
```{r, results='asis'}
## Tabela kable()
   in7 = julia_eval("final_stats")
   in7 %>% my_kbl_foot(header = 10)
```
:::
...
\begin{center}
   Tabela 7: Média de vôos atrasados por Cia estudada (Obtida através do Julia)
\end{center}
